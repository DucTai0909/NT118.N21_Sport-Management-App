...
public class Register extends AppCompatActivity {
    EditText userName, password, fullName, phoneNumber, email,birthday;
    TextView btnLogin;
    RadioGroup gender;
    Button BtnGegister;
    FirebaseAuth fAuth;
...
//                            birthday.setError("Ngày sinh không hợp lệ");
                            birthday.setText("");
                            Toast.makeText(Register.this, "Ngày sinh không hợp lệ", Toast.LENGTH_SHORT).show();
                        }
                    }
                }, year, month, day);
                ...
                .addOnCompleteListener(phoneQueryTask -> {
                    if (phoneQueryTask.isSuccessful()) {
                        if (!phoneQueryTask.getResult().isEmpty()) {
                            ...
                            Toast.makeText(Register.this, "Số điện thoại đã được sử dụng", Toast.LENGTH_SHORT).show();
                        } else {
                            // Kiểm tra trùng lặp tên người dùng
                            db.collection("User")
                                    .whereEqualTo("userName", uUserName)
                                    .get()
                                    .addOnCompleteListener(userNameQueryTask -> {
                                        if (userNameQueryTask.isSuccessful()) {
                                            ...
                                            progressDialog.dismiss();
                                            Toast.makeText(Register.this, "Error " + userNameQueryTask.getException().getMessage(), Toast.LENGTH_SHORT).show();
                                        }
                                    });
                        }
                    } else {
                        progressDialog.dismiss();
                        Toast.makeText(Register.this, "Error " + phoneQueryTask.getException().getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
            ...
            public void onFocusChange(View view, boolean hasFocus) {
                if (!hasFocus) {
                    ...
                    if (TextUtils.isEmpty(uUserName)) {
                        userName.setError("Username không được trống");
                    }
                }
            }
        });
